  ...
  <div class="todo-container">
    <div class="todo-wrap">
      ...
      <TodoFooter
        ...
        :delAllItemChecked="delAllItemChecked"
      />
    </div>
  </div>
</template>
...
step6：删除单个元素(删除一般是敏感操作，所以要多设计环节来防止用户误删)
  * 数组删除的方法：
     - 删一个：splice
     - 删多个：filter
step7：同步更新全部任务个数
step8：同步更新已完成任务个数
  * 可以将全部todos都传给footer，然后计算
  * 也可以在App中计算过后，将值传给footer
  * 一般，数据源在哪，对数据源的操作就放在哪
step9: 全选&全不选
step10：清除已完成任务 
*/
  ...
  data() {
    return {
      todos: [
        ...
        { id: 3, name: "小火车头", isChecked: false },
        { id: 4, name: "小萝卜丁", isChecked: false },
        { id: 5, name: "阿巴阿巴", isChecked: false },
      ],
    };
  },
  computed: {
    ...
    todoItemDone() {
      return this.todos.reduce((p, c) => {
        // 用数组的reduce，初始值为0，为true时加1
        //数据类型强制转换，num + true / false，强制转为 1 / 0 进行数字的计算
